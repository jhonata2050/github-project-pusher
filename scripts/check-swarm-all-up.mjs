...
async function checkAllUp() {
  const server = await getActiveClusterServer();

  console.log("=== STATUS DOS SERVIÇOS SWARM ===");
  ...
  console.log("\n=== TAREFAS PREPARING / STARTING ===");
  const tasks = await SshConnectionManager.execCommand(server, "docker service ps $(docker service ls -q) --filter 'desired-state=running' --format 'table {{.Name}}\t{{.CurrentState}}\t{{.Error}}' | head -n 25", { timeoutMs: 15000 });
  console.log(tasks.out.trim());

  await SshConnectionManager.closeAll();
}

checkAllUp().catch(console.error);
