...
async function checkPostgresError() {
  const server = await getActiveClusterServer();

  console.log("=== LOGS DE app_48f9566be7a07925_db ===");
  const logs = await SshConnectionManager.execCommand(server, "docker service logs --tail 25 app_48f9566be7a07925_db 2>&1");
  console.log(logs.out.trim());

  await SshConnectionManager.closeAll();
}

checkPostgresError().catch(console.error);
