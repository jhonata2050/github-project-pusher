...
async function testCoolifyPatchFields() {
  ...
  const fields = [
    ...
    { post_deployment_command: 'echo ok' },
  ...
  for (const field of fields) {
    ...
    console.log(`Field ${Object.keys(field)[0]}: Status ${res.status}`, await res.text());
  }
}

testCoolifyPatchFields();
