...
async function inspectCoolifyLogs() {
  ...
  console.log('App Container Logs:', logs.data || logs);

  const exec = await coolifyFetch(`/applications/${appUuid}/execute-command`, {
    method: 'POST',
    body: JSON.stringify({ command: 'ls -la /var/www/html /usr/share/caddy' })
  });
  console.log('Command execute result:', exec);
}
