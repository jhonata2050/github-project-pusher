...
async function testDockerfileBuildPack() {
  ...
  const wgetCommands = downloadUrls.map((url, idx) => 
    `wget -qO /tmp/p${idx}.zip "${url}" && unzip -o /tmp/p${idx}.zip -d /var/www/html && rm -f /tmp/p${idx}.zip`
  ).join(' && \\\n    ');

  const dockerfile = `FROM caddy:2-alpine
RUN apk add --no-cache curl unzip wget && \\
    mkdir -p /var/www/html /usr/share/caddy /srv && \\
    ${wgetCommands} && \\
    cp -r /var/www/html/* /usr/share/caddy/ 2>/dev/null || true && \\
    cp -r /var/www/html/* /srv/ 2>/dev/null || true

RUN echo ':80 {\\n    root * /var/www/html\\n    file_server\\n    encode zstd gzip\\n    try_files {path} {path}/ /index.html\\n}' > /etc/caddy/Caddyfile
EXPOSE 80
`;

  console.log('Dockerfile gerado:\n', dockerfile);

  console.log('\nAtualizando aplicação no Coolify para build_pack: dockerfile...');
  ...
  const depRes = await coolifyFetch(`/deploy?uuid=${appUuid}`, { method: 'POST' });
  console.log('Deploy Status:', depRes.status, depRes.data);

  console.log('Aguardando 25 segundos para build do Dockerfile e inicialização...');
