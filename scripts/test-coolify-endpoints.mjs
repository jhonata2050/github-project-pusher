...
async function testCoolifyDeployEndpoints() {
  ...
  // Teste 1: POST /deploy com body
  console.log('Testando POST /deploy com body JSON...');
  const res1 = await coolifyFetch('/deploy', {
    method: 'POST',
    body: JSON.stringify({ uuid: appUuid, force: true })
  });
  console.log('Resultado POST /deploy:', res1);

  // Teste 2: POST /applications/{uuid}/restart
  console.log('Testando POST /applications/{uuid}/restart...');
  const res2 = await coolifyFetch(`/applications/${appUuid}/restart`, {
    method: 'POST'
  });
  console.log('Resultado POST restart:', res2);
}

testCoolifyDeployEndpoints();
