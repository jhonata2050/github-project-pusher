...
export const createTicket = createServerFn({ method: "POST" })
    ...
    try {
        ...
        const whatsappMsg = [
            ...
            `*Urgência:* ${priorityMap[input.priority]}`,
            `*Status:* Aberto`,
            `*Data/Hora:* ${now}`,
            "",
...
export const replyTicket = createServerFn({ method: "POST" })
    ...
    if (ticket) {
        const notificationUserId = isAdmin ? ticket.user_id : null; // Se admin respondeu, notifica o cliente
    ...
    try {
        ...
        const clientName = ticket?.profiles?.full_name || "Cliente";
        const clientEmail = ticket?.profiles?.email || "N/A";
...
export const createServerDA = createServerFn({ method: "POST" })
    ...
    const { data: isAdmin } = await context.supabase.rpc("has_role", { _user_id: context.userId, _role: "admin" });
    if (!isAdmin)
        throw new Error("Unauthorized: Only admins can create servers.");
    const { data, error } = await supabaseAdmin
        .from("servers")
        .insert({
...
export const updateServerDA = createServerFn({ method: "POST" })
    ...
    const { data: isAdmin } = await context.supabase.rpc("has_role", { _user_id: context.userId, _role: "admin" });
    if (!isAdmin)
        throw new Error("Unauthorized: Only admins can update servers.");
    const { data, error } = await supabaseAdmin
        .from("servers")
        .update(patch)
        .eq("id", input.id)
...
export const deleteServerDA = createServerFn({ method: "POST" })
    ...
    const { data: isAdmin } = await context.supabase.rpc("has_role", { _user_id: context.userId, _role: "admin" });
    if (!isAdmin)
        throw new Error("Unauthorized: Only admins can delete servers.");
