...
export const logPublicAuthEvent = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    try {
      ...
      console.warn("[Audit] Ignorando log de auditoria: chave administrativa indisponível.");
...
export const logSessionEvent = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    try {
      ...
      console.warn("[Audit] Ignorando log de auditoria de sessão: chave administrativa indisponível.");
      return { success: false };
    }
  });