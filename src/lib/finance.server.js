...
export async function placeOrder(userId, data) {
    ...
    const price = product.product_prices?.find((p) => p.cycle === data.billingCycle && p.is_active);
    if (!price)
        throw new Error("Preço não encontrado para este ciclo");
    ...
    if (data.couponCode) {
        const { data: coupon } = await supabaseAdmin
            ...
            .maybeSingle();
        if (coupon) {
            const validDate = !coupon.valid_until || new Date(coupon.valid_until) > new Date();
            const validUses = !coupon.max_uses || (coupon.used_count || 0) < coupon.max_uses;
    ...
    const { data: invoice, error: iError } = await supabaseAdmin
        ...
        .single();
    if (iError || !invoice)
        throw new Error("Falha ao criar a fatura");
...
export async function fetchInvoiceDetails(userId, id) {
    ...
    if (isAdmin) {
        const { data: adminInvoice, error: adminError } = await supabaseAdmin
            ...
            .eq("id", id)
            .single();
        if (adminError || !adminInvoice)
            throw new Error("Fatura não encontrada");
        return adminInvoice;
    }
    throw new Error("Fatura não encontrada ou acesso negado");
}
export async function processProvisioning(invoiceId) {
        ...
        if (product?.directadmin_package) {
            ...
            try {
                ...
                try {
                    ...
                    await notifyAdminWhatsApp(`🚀 *Serviço Provisionado*\n\n*Produto:* ${product.name}\n*Cliente:* ${profile?.full_name}\n*Domínio:* ${domain}`, "service_activation");
                }
                catch (e) {
 ...
 */
export async function handlePaymentSuccess(invoiceId, gatewayName, externalReference) {
    try {
        ...
        if (externalReference) {
            ...
            if (tUpdateError)
                console.warn(`[Finance] Aviso: Não foi possível atualizar transação ${externalReference}:`, tUpdateError.message);
        }
        ...
        // Notificar Admin
        await notifyAdminWhatsApp(`💰 *Pagamento Confirmado*\n\n*Fatura:* #${invoiceId}\n*Valor:* ${amountStr}\n*Gateway:* ${gatewayName}\n*Cliente:* ${profile?.full_name || "N/A"}`, "payment_success");
        // Notificar Cliente
        if (profile?.phone) {
            await sendWhatsAppMessage({
                to: profile.phone,
        ...
        await supabaseAdmin.from("audit_logs").insert({
            ...
            metadata: { invoiceId, gatewayName, externalReference }
        });
        return { success: true };
    }
    catch (error) {
        ...
        await supabaseAdmin.from("audit_logs").insert({
            ...
            metadata: { invoiceId, gatewayName, externalReference, error: error.message }
        });
        throw error;
    }
}
