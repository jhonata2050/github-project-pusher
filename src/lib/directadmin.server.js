...
export async function callDA({ hostname, apiUser, apiToken, command, method = 'GET', params = {} }) {
    ...
    if (!apiUserRaw || !apiTokenTrimmed) {
        throw new Error(`Credenciais do servidor ${hostname} não configuradas. Preencha o "Usuário API" (formato admin|EqsamKey) e o "Token API" em Sistema > Servidores.`);
    }
    ...
    try {
        ...
        if (!response.ok) {
            ...
            if (response.status === 403 && (errorText.includes('Imunify360') || errorText.includes('bot-protection') || errorText.includes('shield-root'))) {
                throw new Error(`O Imunify360 do servidor ${hostname} bloqueou a requisição (proteção anti-bot). ` +
            ...
            if (response.status === 403 && /not allowed|Access Denied/i.test(errorText)) {
                throw new Error(`A chave de API do DirectAdmin não tem permissão para o comando "${command}". ` +
            ...
            if (response.status === 401) {
                ...
                throw new Error(`Falha na autenticação (401): O DirectAdmin não reconheceu as credenciais. ` +
        ...
        if (error instanceof Error && /Imunify360/i.test(error.message))
            throw error;
...
function normalizePackageList(result) {
    ...
    if ('list' in result && Array.isArray(result.list)) {
        return result.list.filter((item) => typeof item === 'string');
    }
    const packages = [];
...
export async function getDAPackages(serverId) {
    ...
    if (packages.length === 0)
        throw new Error('A conexão foi aceita, mas nenhum pacote de usuário foi retornado pelo DirectAdmin.');
    return packages;
}
export async function getDACapabilities(serverId) {
    const { data: server, error } = await supabaseAdmin
        .from('servers')
        .select('*')
        .eq('id', serverId)
...
export async function testDAConnectionDetails(serverId) {
    const { data: server, error } = await supabaseAdmin
        .from('servers')
...
export async function createDAAccount(serverId, details) {
    const { data: server, error } = await supabaseAdmin
        .from("servers")
        .select("*")
        .eq("id", serverId)
...
export async function suspendDAAccount(serverId, username) {
    const { data: server, error } = await supabaseAdmin
        .from("servers")
        .select("*")
        .eq("id", serverId)
...
export async function deleteDAAccount(serverId, username) {
    const { data: server, error } = await supabaseAdmin
        .from("servers")
        .select("*")
        .eq("id", serverId)
...
function isValidDirectAdminLoginUrl(value) {
    ...
    if (containsMarker && containsPath)
        return true;
    try {
...
function parseDirectAdminLoginUrl(response, serverHostname) {
...
export async function checkDAUserExists(serverId, username, serviceId) {
    const { data: server, error } = await supabaseAdmin
        .from("servers")
        .select("*")
        .eq("id", serverId)
    ...
    try {
        ...
        if (errorStr.includes("You don't have control over that user") || errorStr.includes("Cannot show user")) {
            ...
            if (serviceId) {
                ...
                await notifyAdminWhatsApp(`⚠️ *CONFLITO DE SEGURANÇA (DA)*\n\n*Domínio:* ${username}\n*Servidor:* ${server.hostname}\n*Ação:* Serviço suspenso automaticamente para evitar acesso indevido.\n\nVerifique o painel de auditoria.`, "security_conflict");
            }
            return false;
        }
...
export async function getDASession(serverId, username, redirectUrl) {
    const { data: server, error } = await supabaseAdmin
        .from("servers")
        .select("*")
        .eq("id", serverId)
        .single();
    if (error || !server)
        throw new Error("DA_INVALID_SERVER");
    ...
    try {
        ...
        if (errorMsg.includes('401'))
            throw new Error('DA_AUTHENTICATION_ERROR');
        if (errorMsg.includes('IP') || errorMsg.includes('whitelist'))
            throw new Error('DA_LOGIN_KEY_IP_NOT_ALLOWED');
        if (errorMsg.includes('permission') || errorMsg.includes('perm'))
            throw new Error('DA_PERMISSION_ERROR');
