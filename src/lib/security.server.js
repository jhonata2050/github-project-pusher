 ...
 */
export async function validateDASSORequest(userId, username, serverId) {
    ...
    const result = await directAdminModule.callDA({
    ...
    if (result.usertype !== 'user' && !isAdmin) {
        ...
        try {
            const { notifyAdminWhatsApp } = await import("./whatsapp.server");
            await notifyAdminWhatsApp(`🚨 *ALERTA CRÍTICO DE ESCALONAMENTO*\n\nTentativa de acesso administrativo via SSO bloqueada!\n\n*Usuário Lovable:* ${userId}\n*Usuário DA:* ${cleanUsername}\n*Nível Remoto:* ${result.usertype}\n\nO sistema impediu a sessão automaticamente.`, "security_critical");
        }
        catch (e) { }
        throw new Error("DA_DIRECTADMIN_BLOCKED");
    }
