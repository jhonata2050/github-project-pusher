...
async function getContaboToken() {
    ...
    if (!res.ok) {
        ...
        if (detail.toLowerCase().includes('invalid user credentials') || (errorBody && errorBody.error === 'invalid_grant')) {
            throw new Error("Contabo recusou as credenciais (usuário/senha da API inválidos). No Painel do Cliente Contabo, em 'API', use o E-mail da API e a Senha da API (não a senha da sua conta), e confira o Client ID/Secret.");
        }
...
export async function performContaboAction(instanceId, action, userId) {
...
export async function getContaboProductTypes() {
    try {
        ...
        allProducts.forEach((p) => {
            ...
            const cpuSpec = specs.find((s) => s.type === 'cpu' || s.title?.toLowerCase().includes('cpu'));
            const ramSpec = specs.find((s) => s.type === 'ram' || s.title?.toLowerCase().includes('ram'));
            const diskSpec = specs.find((s) => s.type === 'storage' || s.title?.toLowerCase().includes('ssd') || s.title?.toLowerCase().includes('nvme') || s.title?.toLowerCase().includes('disk'));
            ...
            if (name.includes("vds"))
                category = "VDS (Dedicated Servers)";
            else if (name.includes("vps") && name.includes("storage"))
...
export async function provisionContaboVPS(serviceId, config) {
    console.log("Provisioning Contabo VPS for service:", serviceId);
}
export async function getContaboInstanceDetails(externalId) {
    const token = await getContaboToken();
...
export async function getContaboInstanceStats(externalId) {
    try {
        const token = await getContaboToken();
