...
import { Package, Plus, Search, Store, Edit2, Server, Copy } from "lucide-react";
...
function ProductsPage() {
    ...
    const products = useQuery({
        ...
        queryFn: async () => {
            const { data, error } = await supabase
                .from("products")
                .select("id, name, slug, description, directadmin_package, external_id, disk_quota_mb, is_visible, sort_order, product_type, group_id, immediate_purchase, product_groups(name), product_prices(cycle, price, is_active)")
                .order("sort_order");
            if (error)
                throw error;
            return data;
    ...
    useEffect(() => {
        if (!selectedServer && servers.data && servers.data.length > 0) {
            setSelectedServer(servers.data[0].id);
        }
    ...
    const filtered = (products.data ?? []).filter((p) => p.name.toLowerCase().includes(term.trim().toLowerCase()));
    ...
    const handleSave = () => {
        updateMutation.mutate({
            ...
            prices: editingProduct.prices.map((p) => ({
                cycle: p.cycle,
    ...
    return (<AppShell area="admin" breadcrumb={<>
      ...
      <div className="mt-6 flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-56">
          ...
          <Input value={term} onChange={(e) => setTerm(e.target.value)} placeholder="Pesquisar" className="h-11 rounded-xl pl-9"/>
        </div>
      ...
      {products.isLoading ? (<div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {[0, 1, 2].map((i) => (<Skeleton key={i} className="h-40 rounded-2xl"/>))}
        </div>) : filtered.length === 0 ? (<p className="py-24 text-center text-sm text-muted-foreground">Nenhum produto encontrado</p>) : (<div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {filtered.map((product) => {
                const monthly = product.product_prices?.find((p) => p.cycle === "monthly" && p.is_active);
                return (<article key={product.id} className="group relative rounded-2xl border border-border p-5 transition-all hover:shadow-[var(--shadow-card)]">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button variant="ghost" size="icon" className="size-8 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity" onClick={() => handleEdit(product)}>
                ...
                <dl className="mt-4 space-y-1 text-xs text-muted-foreground">
                  ...
                  {product.immediate_purchase && (<div className="flex justify-between items-center mt-1">
                      <dt className="text-brand font-medium">Link de Venda</dt>
                      <dd>
                        <Button variant="ghost" size="icon" className="size-6 h-6 w-6" onClick={(e) => {
                            e.stopPropagation();
      ...
      <Dialog open={!!editingProduct} onOpenChange={(open) => !open && setEditingProduct(null)}>
        <DialogContent className="max-w-2xl rounded-3xl border-none shadow-2xl max-h-[90vh] overflow-y-auto">
          ...
          {editingProduct && (<div className="space-y-6 py-4">
              <div className="grid gap-4 md:grid-cols-2">
                <div className="space-y-2">
                  <Label>Nome do Plano</Label>
                  <Input value={editingProduct.name} onChange={e => setEditingProduct({ ...editingProduct, name: e.target.value })} placeholder="Ex: Hospedagem Start" className="rounded-xl"/>
                </div>
                <div className="space-y-2">
                  <Label>Tipo de Produto</Label>
                  <Select value={editingProduct.product_type} onValueChange={val => setEditingProduct({ ...editingProduct, product_type: val })}>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div className="space-y-2">
                  <Label>Grupo</Label>
                  <Select value={editingProduct.group_id || ""} onValueChange={val => setEditingProduct({ ...editingProduct, group_id: val })}>
                    <SelectTrigger className="rounded-xl">
                      <SelectValue placeholder="Selecione um grupo"/>
                    </SelectTrigger>
                    <SelectContent className="rounded-xl border-none shadow-xl">
                      {productGroups.data?.map((g) => (<SelectItem key={g.id} value={g.id}>{g.name}</SelectItem>))}
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-2">
                  <Label>Ordem de Exibição</Label>
                  <Input type="number" value={editingProduct.sort_order} onChange={e => setEditingProduct({ ...editingProduct, sort_order: Number(e.target.value) })} className="rounded-xl"/>
                </div>
              </div>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div className="flex items-center justify-between border border-border rounded-xl p-4 bg-muted/20">
                  ...
                  <Switch checked={editingProduct.immediate_purchase} onCheckedChange={val => setEditingProduct({ ...editingProduct, immediate_purchase: val })}/>
                </div>
                {editingProduct.immediate_purchase && editingProduct.id && (<div className="flex items-center gap-2 border border-border rounded-xl p-4 bg-brand/5">
              ...
              <div className="space-y-2">
                  ...
                  <Input type="number" value={editingProduct.disk_quota_mb || ""} onChange={e => setEditingProduct({ ...editingProduct, disk_quota_mb: Number(e.target.value) })} placeholder="Ex: 1024 para 1GB" className="rounded-xl"/>
                </div>
                <div className="flex items-center justify-between pt-8">
                  <Label>Produto Visível</Label>
                  <Switch checked={editingProduct.is_visible} onCheckedChange={val => setEditingProduct({ ...editingProduct, is_visible: val })}/>
                </div>
              </div>

              <div className="space-y-2">
                <Label>Descrição</Label>
                <Textarea value={editingProduct.description || ""} onChange={e => setEditingProduct({ ...editingProduct, description: e.target.value })} className="rounded-xl min-h-[80px]"/>
              </div>
              ...
              <div className="rounded-2xl border border-border p-4 bg-muted/30">
                ...
                {editingProduct.product_type === 'vps' ? (<div className="space-y-2">
                    <Label>Plano Contabo (Product ID)</Label>
                    <Select value={editingProduct.external_id || ""} onValueChange={val => setEditingProduct({ ...editingProduct, external_id: val, directadmin_package: val })}>
                      <SelectTrigger className="rounded-xl">
                        <SelectValue placeholder={contaboPlans.isLoading ? "Carregando planos da Contabo..." :
                    contaboPlans.error ? "Erro ao carregar (verifique API)" :
                        "Selecione o plano VPS"}/>
                      </SelectTrigger>
                      <SelectContent className="rounded-xl border-none shadow-xl">
                        {contaboPlans.data?.map((cat) => (<div key={cat.category}>
                            <div className="px-2 py-1.5 text-xs font-bold text-brand bg-brand/5 uppercase tracking-wider sticky top-0">
                              {cat.category}
                            </div>
                            {cat.items.map((plan) => (<SelectItem key={plan.productId} value={plan.productId}>
                                {plan.name} — {plan.vCpu} / {plan.ramTitle} / {plan.diskGb}
                              </SelectItem>))}
                          </div>))}
                        {(!contaboPlans.data || contaboPlans.data.length === 0) && !contaboPlans.isLoading && (<div className="p-4 text-xs text-center text-muted-foreground">
                            {contaboPlans.error ? (<div className="text-destructive font-medium">
                                Falha na API Contabo. <br />
                                Certifique-se de que as credenciais em Sistema {"/ >"} Servidores estão corretas.
                              </div>) : "Nenhum plano encontrado na API"}
                          </div>)}
                      </SelectContent>
                    </Select>
                    ...
                    <div className="space-y-2">
                      ...
                      <Select value={selectedServer} onValueChange={setSelectedServer}>
                        ...
                        <SelectContent className="rounded-xl border-none shadow-xl">
                          {servers.data?.map((s) => (<SelectItem key={s.id} value={s.id}>
                              {s.hostname ?? s.name ?? s.ip_address ?? s.id}
                            </SelectItem>))}
                          {(!servers.data || servers.data.length === 0) && (<div className="p-2 text-xs text-center text-muted-foreground">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select value={editingProduct.directadmin_package || ""} onValueChange={val => setEditingProduct({ ...editingProduct, directadmin_package: val })}>
                        <SelectTrigger className="rounded-xl">
                          <SelectValue placeholder={daPackages.isLoading ? "Carregando..." : "Selecione um pacote"}/>
                        </SelectTrigger>
                        <SelectContent className="rounded-xl border-none shadow-xl">
                          {daPackages.data?.map((pkg) => (<SelectItem key={pkg} value={pkg}>{pkg}</SelectItem>))}
                          {(!daPackages.data || daPackages.data.length === 0) && !daPackages.isLoading && (<div className="p-2 text-xs text-center text-muted-foreground">
                              {daPackages.error
                        ? daPackages.error.message
              ...
              <div className="space-y-4">
                ...
                const priceObj = editingProduct.prices.find((p) => p.cycle === cycle) || { cycle, price: 0, is_active: false };
                return (<div key={cycle} className="flex items-center gap-4 rounded-xl border border-border p-3 bg-white">
                        <div className="flex-1">
                          <Label className="capitalize text-xs">{CYCLE_LABELS[cycle]}</Label>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="text-sm font-bold">R$</span>
                          <Input type="number" value={priceObj.price} onChange={e => {
                        ...
                        <Switch checked={priceObj.is_active} onCheckedChange={val => {
                        const newPrices = [...editingProduct.prices];
                        const idx = newPrices.findIndex(p => p.cycle === cycle);
                        if (idx > -1) {
          ...
          <DialogFooter className="mt-6 flex flex-row gap-2">
            ...
            <Button className="flex-1 rounded-2xl bg-brand text-brand-foreground hover:bg-brand/90" onClick={handleSave} disabled={updateMutation.isPending}>
